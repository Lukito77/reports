...
export default function AdminReportsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{lang === 'ka' ? 'განაცხადების მართვა' : 'Reports'}</h1>
              ...
              <div className="space-y-2 border-t border-slate-200 pt-3">
                ...
                {user?.role === 'ADMIN' && (
                  <div className="border-t border-slate-200 pt-3">
                    ...
                    <p className="mt-1 text-[11px] text-slate-400">{t.admin.deleteHint}</p>
                  </div>
                )}
                <p className="text-[11px] text-slate-400">{t.admin.approvalHint}</p>
              </div>
            </div>
          ) : (
